...
export default function FacialEmotionDetection({ onDetected, accentClass }: FacialEmotionDetectionProps) {
  ...
  const handleStartCamera = async () => {
    ...
    try {
      ...
      setCameraError(true);
      setStreamActive(true); // fall back to mock stream visual
    }
  };
    ...
    <div className="flex flex-col items-center">
      <div className="w-full max-w-sm rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 relative aspect-video flex items-center justify-center text-slate-500 shadow-inner">
        ...
        {streamActive ? (
          <>
            {cameraError ? (
              /* Simulated sensory scanner visuals */
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 p-4">
                ...
                <span className="text-[9px] text-slate-500 block mt-1">Camera disabled or unavailable. Scanning mock matrix.</span>
              </div>
            ) : (
              /* Real Camera stream */
              <video
            ...
            {isScanning && (
              <div 
                className="absolute left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-purple-400 to-transparent shadow-[0_0_8px_#c084fc] z-10 pointer-events-none animate-custom-scan"
                style={{
                  top: `${scanProgress}%`
                }}
              />
      ...
      {/* Detected results board */}
      {detectedEmotion && (
        <div className="mt-5 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200/50 dark:border-emerald-900/30 p-4 rounded-xl text-center max-w-sm animate-fade-in relative overflow-hidden">
