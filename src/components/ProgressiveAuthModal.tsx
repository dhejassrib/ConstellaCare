...
//                 <button
//                   onClick={() => {
//                     alert("✨ Seamless Google Account Integration! Dispatched secure tokens to preserve your treatment timeline orbits.");
//                     onClose();
//                   }}
...
//                 <button
//                   onClick={() => {
//                     const email = prompt("Enter email description to bind your progress:", "sarah@constella.com");
//                     if (email) {
//                       alert(`✉️ OTP Magic access link coordinates transmitted to ${email}! Check details.`);
//                       onClose();
//                     }
//                   }}
...
//                 <button
//                   onClick={onClose}
//                   className="w-full py-2.5 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-300 font-bold block transition"
//                 >
//                   Continue as Guest
//                 </button>
//               </div>
              ...
              <div className="space-y-4 pb-1.5 min-h-[160px] flex flex-col justify-center">
                ...
                {simState === 'google' && (
                  <div className="text-left space-y-3 w-full">
                    <span className="text-[10px] font-black text-pink-500 block uppercase tracking-wider">Simulated Google Account Picker</span>
                    <p className={`text-[11px] leading-relaxed ${theme === 'dark' ? 'text-purple-300' : 'text-slate-650'}`}>Select an account to authorize secure credentials link:</p>
                    <div className="space-y-2 mt-2">
                      {[
                        ...
                        <button
                          ...
                          className={`w-full p-2.5 rounded-2xl border flex items-center gap-3 text-left transition hover:scale-[1.01] cursor-pointer ${theme === 'dark' ? 'bg-[#1b1536]/80 border-purple-500/25 text-slate-100 hover:bg-purple-950/20' : 'bg-slate-50 border-purple-200 text-slate-800 hover:bg-purple-50'}`}
                        >
                    ...
                    {loading && (
                      <div className="flex items-center justify-center gap-2 mt-3 text-xs text-pink-500 font-bold animate-pulse">
                        <span className="animate-spin text-sm">🌀</span> Linking cloud orbits...
                      </div>
                    )}
                    <button
                      onClick={() => setSimState('none')}
                      className="text-[10px] px-3 py-1 bg-slate-100 dark:bg-purple-950/40 text-slate-600 dark:text-purple-300 rounded-lg font-bold mt-2 hover:bg-slate-200 dark:hover:bg-purple-900/40"
                    >
                      ← Back
                    </button>
                  </div>
                ...
                {simState === 'email' && (
                  ...
                  >
                    <span className="text-[10px] font-black text-pink-500 block uppercase tracking-wider">Email OTP Dispatcher</span>
                    <p className={`text-[11px] leading-relaxed ${theme === 'dark' ? 'text-purple-300' : 'text-slate-655'}`}>Input email to trigger an instant OTP verification key:</p>
                    ...
                    >
                      {loading ? "Sending safe token..." : "Send Verification Token ✉️"}
                    </button>
                    
                    <button
                      type="button"
                      onClick={() => setSimState('none')}
                      className="text-[10px] px-3 py-1 bg-slate-100 dark:bg-purple-950/40 text-slate-600 dark:text-purple-300 rounded-lg font-bold hover:bg-slate-200 dark:hover:bg-purple-900/40"
                    >
                      ← Back
                    </button>
                  </form>
                  ...
                  <form
                    onSubmit={(e) => {
                      ...
                      setTimeout(() => {
                        ...
                        if (onAuthSuccess) {
                          onAuthSuccess(emailInput, displayName);
                        }
                        setSimState('success');
                      }, 1200);
                    }}
                    className="text-left space-y-3 w-full"
                  >
                    <span className="text-[10px] font-black text-pink-500 block uppercase tracking-wider">Verify Access Sequence</span>
                    <p className={`text-[11px] leading-relaxed ${theme === 'dark' ? 'text-purple-300' : 'text-slate-655'}`}>Transmitted authentication digits to <b className="font-mono text-pink-500">{emailInput}</b>. Type any 4 digits:</p>
                ...
                {simState === 'success' && (
                  <div className="py-4 flex flex-col items-center justify-center text-center space-y-3 w-full">
                    ...
                    <button
                      onClick={onClose}
                      className="px-5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-[11px] font-bold cursor-pointer"
                    >
                      Enter Aligned Sky
                    </button>
                  </div>
